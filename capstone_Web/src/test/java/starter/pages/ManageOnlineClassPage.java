...
public class ManageOnlineClassPage extends PageObject {
    private String classOnline;
    private By manageOnlineClassMenu(){
        return By.xpath("//li[text()='Manage Online Class']");
    }
    private By crudButton(String crud_button, int row){
        return By.xpath("//tr[" + row + "]//button[@id='btn-" + crud_button + "-online-class']");
    }
    private By addButton(){
        return By.id("btn-add-online-class");
    }
    ...
    private By cancelButtonEdit(){
        return By.id("btn-cancel-edit-class-offline-class");
    }
    private By saveButtonAdd(String save_button){
        return By.id("btn-save-" + save_button + "-offline-class");
    }

    private By saveButtonEdit(){
        return By.id("btn-save-edit-class-offline-class");
    }
    ...
    private By manageOnlineClassText(){
        return By.xpath("//h1[text()='Manage Online Class']");
    }

    private By fldNameClassAdd(String crud_button){
        return By.id("fld-" + crud_button + "-name-offline-class");
    }

    private By fldNameClassEdit(){
        return By.id("fld-edit-name-class-offline-class");
    }
    ...
    @Step
    public void clickManageOnlineClass(){
        $(manageOnlineClassMenu()).click();
    }

    @Step
    public boolean validateOnManageOnlineClassMenu(){
        return $(manageOnlineClassText()).isDisplayed();
    }

    @Step
    public boolean viewOnlineClassPopUpShow(){
    ...
    @Step
    public void clickAddOnOnlineClass(){
        $(addButton()).click();
    }

    @Step
    public boolean validateAddOnlineClassPopUpShow(String crud_button){
        return $(saveButtonAdd(crud_button)).isDisplayed();
    }

    @Step
    public void inputDataForAddOnlineClass(String name_class, String trainer, String date, int time, String location, int price, String desc, String result, String crud_button){
        WebDriverWait wait = new WebDriverWait(getDriver(), Duration.ofSeconds(10));
        wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.className("ant-modal-body")));
        WebElement modal = getDriver().findElement(By.className("ant-modal-body"));
        if (result.equals("success")){
            modal.findElement(fldNameClassAdd(crud_button)).sendKeys(name_class);
    ...
    public void clickSaveButton(String crud_button){
        ...
        if (crud_button.equals("edit")){
            modal.findElement(saveButtonEdit()).click();
        } else if (crud_button.equals("add")){
            modal.findElement(saveButtonAdd(crud_button)).click();
        }

    }

    @Step
    public void inputDataForEditOnlineClass(String name_class, String trainer, String date, int time, String location, int price, String desc, String result, String crud_button){
        WebDriverWait wait = new WebDriverWait(getDriver(), Duration.ofSeconds(10));
        wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.className("ant-modal-body")));
        WebElement modal = getDriver().findElement(By.className("ant-modal-body"));
        if(result.equals("success")){
            modal.findElement(fldNameClassEdit()).sendKeys(name_class, Keys.chord(Keys.CONTROL, "a"));
            modal.findElement(fldNameClassEdit()).sendKeys(name_class, Keys.ENTER);
    ...
    @Step
    public boolean validateResultAfterEditOnlineClass(String result, int row, String name_class){
//        if (result.equals("success")){
...
//        }
    ...
    public boolean validateDataWasDeletedSuccessfully(int row) throws Exception{
        classOnline = FileUtils.readFileToString(new File(System.getProperty("user.dir") + "//src//test//resources//filejson//class.json"), StandardCharsets.UTF_8);
        return $(nameClassText(row)).getText().equals(classOnline);
    }
